...
def compute_acc(masks_true, predictions):
    matrix = confusion_matrix(y_true=np.array(masks_true).flatten(), y_pred=np.array(predictions).flatten())
...
def main():
    ...
    # ACC
    accuracies = []
    batch_gt = masks_true[i].cpu().numpy().flatten()
    batch_pred = mask.flatten()
    batch_acc = compute_acc(batch_gt, batch_pred)
    ...
    recall = compute_recall(gt_mask, mask)
