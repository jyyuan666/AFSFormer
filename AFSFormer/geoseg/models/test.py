...
class PatchExpand(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dim_scale == 1:
            ...
            x = self.conv(x.permute(0, 3, 1, 2).contiguous()).permute(0, 2, 3, 1).contiguous().view(B, -1,
                                                                                                    C // 4).contiguous()
            x = self.norm(x)
            return x, H * 2, W * 2
...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            x_1 = self.act(
                self.norm1(self.sr1(x_).reshape(B, C, -1).permute(0, 2, 1))).contiguous()  # torch.Size([1, 256, 64])
            x_2 = self.act(
                self.norm2(self.sr2(x_).reshape(B, C, -1).permute(0, 2, 1))).contiguous()  # torch.Size([1, 1024, 64])
...
class ShuntedTransformerSys(nn.Module):
    def __init__(self, img_size=512, patch_size=4, in_chans=3, num_classes=6,
        ...
        dpr_up = [x.item() for x in
                  torch.linspace(0, drop_path_rate, sum(self.up_depths))]  # stochastic depth decay rule
        cur = 0
    ...
    def forward_up_features(self, x, fms, attn_maps):
        ...
        for i in range(self.up_num_stages):
            ...
            if i != 0:
                ...
                attns = attn_conv(
                    attn_maps[self.num_stages - i - 1].permute(0, 2, 1).contiguous().unsqueeze(2)).squeeze(2).permute(0,
                                                                                                                      2,
                                                                                                                      1).contiguous()
...
class ShuntedUnet(nn.Module):
    def __init__(self, img_size=512, num_classes=6, zero_head=False, vis=False):
        ...
        self.shunted_unet = ShuntedTransformerSys(patch_size=4, num_classes=self.num_classes,
                                                  embed_dims=[64, 128, 256, 512], num_heads=[2, 4, 8, 16],
                                                  num_heads_up=[16, 8, 4, 2], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
def create_shunted_unet_model(pretrained=True,
                              ckpt_path='/home/zrh/code/ckpt/ckpt_T.pth', n_classes=2):
...
model = create_shunted_unet_model(pretrained=False, n_classes=2)
y = model(x)
