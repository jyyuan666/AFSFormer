...
class LocalFeatureEnhance(nn.Module):
    def __init__(self,
                 ...
                 norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.Local1 = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1, groups=in_channels)
...
class UpsampleBlock(nn.Module):
    def __init__(self,
        ...
        self.dwconv = nn.Conv2d(in_channels=in_dim, out_channels=out_dim, kernel_size=3, padding=1, groups=1)
...
class get(nn.Module):
    def __init__(self,
        ...
        self.eps = eps
        self.pre_conv1 = nn.Conv2d(in_channels=512, out_channels=64, kernel_size=1)
        self.pre_conv2 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1)
        self.pre_conv3 = nn.Conv2d(in_channels=256, out_channels=64, kernel_size=1)

    def forward(self, res2, res3, res4):
        res4 = F.interpolate(res4, scale_factor=2, mode='bilinear', align_corners=False)
        res2 = self.down(res2)
        weights = nn.ReLU()(self.weights)
        fuse_weights = weights / (torch.sum(weights, dim=0) + self.eps)
        x = fuse_weights[0] * self.pre_conv1(res4) + fuse_weights[1] * self.pre_conv3(res3) + fuse_weights[2] * self.pre_conv2(res2)
...
class FeatureFuse(nn.Module):
    def __init__(self,
        ...
        self.pre_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1)
...
class FeatureRefinement(nn.Module):
    def __init__(self,
        ...
        self.DWconv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1, groups=in_channels)
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1)
        self.weights = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        self.eps = 1e-8

        self.Average_pool = nn.AdaptiveAvgPool2d(1)
        self.channel_reduce = nn.Conv2d(in_channels, out_channels // 8, kernel_size=1)
        self.channel_expand = nn.Conv2d(out_channels // 8, out_channels, kernel_size=1)
    ...
    def channels_refinement(self, x):
        ...
        x_ = self.channel_expand(self.channel_reduce(x_))
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.pre_conv2 = nn.Conv2d(encoder_channels[-2], decoder_channels, kernel_size=1)
        self.pre_conv3 = nn.Conv2d(encoder_channels[-3], decoder_channels, kernel_size=1)
        self.con = nn.Conv2d(in_channels=64, out_channels=decoder_channels, kernel_size=3,padding=1)
        self.get = get(in_dim=decoder_channels, out_dim=decoder_channels)

        self.up = UpsampleBlock()

        self.p2 = FeatureFuse(in_channels=decoder_channels, out_channels=decoder_channels)
        self.b2 = Block(in_dim=decoder_channels, num_heads=16, window_size=8)

        self.p3 = FeatureFuse(in_channels=decoder_channels, out_channels=decoder_channels)
        self.b3 = Block(in_dim=decoder_channels, out_dim=decoder_channels)


        self.fr = FeatureRefinement(in_channels=decoder_channels, out_channels=decoder_channels)



        self.segmentation_head = nn.Sequential(DWConvBNRelu(in_channels=decoder_channels, out_channels=decoder_channels),
                                               nn.Dropout2d(p=dropout, inplace=True),
                                               nn.Conv2d(decoder_channels, num_classes, kernel_size=1, padding=0))
    ...
    def forward(self, res1, res2, res3, res4):
        ...
        res3_ = self.pre_conv2(res3) + self.up(x)

        x = self.p2(x, res3_)

        x = self.con(x)

        res2_ = self.pre_conv3(res2) + self.up(x)
        x =self.p3(x, res2_)

        x = self.b3(x)
        x = F.interpolate(x, scale_factor=2, mode='bilinear', align_corners=False)

        get = self.get(res2, res3, res4)
        get = self.up(self.up(get))
