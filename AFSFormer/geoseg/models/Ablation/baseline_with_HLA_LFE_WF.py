...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.attnb1 = Block(in_dim=decoder_channels, num_heads=16, window_size=8)
        self.b1 = FeatureFuse(in_channels=encoder_channels[-2], out_channels=decoder_channels)

        self.attnb2 = Block(in_dim=decoder_channels, num_heads=16, window_size=8)

        self.b2 = FeatureFuse(in_channels=encoder_channels[-3], out_channels=decoder_channels)

        self.attnb3 = Block(in_dim=decoder_channels, num_heads=16, window_size=8)
        self.b3 = FeatureFuse(in_channels=encoder_channels[-4], out_channels=decoder_channels)
...
class Baseline_with_HLA_LFE_WF(nn.Module):
    def __init__(self,
                 decode_channels=64,
                 dropout=0.1,
