...
import torchsummary
from ops.bra_nchw import regional_routing_attention_torch
import timm
...
class WF(nn.Module):
    def __init__(self, in_channels=64, decode_channels=64, eps=1e-8):
        super(WF, self).__init__()
        self.pre_conv = DWConv(in_channels, decode_channels, kernel_size=1, padding=0)

        self.weights = nn.Parameter(torch.ones(2, dtype=torch.float32), requires_grad=True)
        self.eps = eps
        self.post_conv = DWConvBNRelu(decode_channels, decode_channels, kernel_size=3)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.b1 = Block(dim=decoder_channels, num_heads=16, window_size=8)

        self.b2 = Block(dim=decoder_channels, num_heads=16, window_size=8)

        self.p2 = WF(encoder_channels[-2], decoder_channels)

        self.b3 = Block(dim=decoder_channels, num_heads=16, window_size=8)
        self.p3 = WF(encoder_channels[-3], decoder_channels)
