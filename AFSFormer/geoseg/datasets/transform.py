...
class RandomScale(object):
    ...
    def __call__(self, img, mask):
        ...
        scale_amt = 1.0
        if self.mode == 'value':
            scale_amt = np.random.choice(self.scale_list, 1) # 1是从scalelist中选一个
        elif self.mode == 'range':
            scale_amt = random.uniform(self.scale_list[0], self.scale_list[-1])
        h = int(scale_amt * oh)
        w = int(scale_amt * ow)
        return img.resize((w, h), Image.BICUBIC), mask.resize((w, h), Image.NEAREST) # PIL.Iamge.resize()中的Image.BICUBIC参数是默认的重采样滤波器
       # 对于图像模式是L或者P，只能用Image.NEAREST
...
class SmartCropV1(object):
    def __init__(self, crop_size=512,
                 max_ratio=0.75,
                 ignore_index=12, nopad=False): # ignore index 为什么设置为12
